...
/**
 * Veo 3.1 숏폼 클립 비디오 생성 스크립트 (9:16)
 *
 * videoPrompts-short.json을 읽어 각 8초 클립에 대응하는
 * 비디오를 Veo 3.1 API로 생성하고 public/videos/clips-short/ 에 저장합니다.
 *
 * ⚠️ 고정 포맷:
 *   - 비율: 9:16 portrait
 *   - 해상도: 720p
...
import type { VideoPromptShort } from "./generateVideoPrompts-short";
...
const OUTPUT_DIR = path.join(__dirname, "../../public/videos/clips-short");
const PROMPTS_JSON = path.join(__dirname, "videoPrompts-short.json");

// Veo 3.1: fixed 8 seconds per clip, 9:16 portrait
...
async function submitVideoGeneration(prompt: string): Promise<string> {
  ...
  if (!operationName) {
    throw new Error("No operation name in Veo response: " + JSON.stringify(data));
  }
...
async function pollOperation(operationName: string): Promise<string> {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (data.done) {
      ...
      if (samples.length === 0) {
        throw new Error("No generated samples in response: " + JSON.stringify(data));
      }
  ...
  throw new Error(`Timeout: operation ${operationName} did not complete in 10 minutes`);
}

async function downloadVideo(videoUri: string, outputPath: string): Promise<void> {
...
async function main() {
  if (!fs.existsSync(PROMPTS_JSON)) {
    console.error("❌ videoPrompts-short.json이 없습니다. generateVideoPrompts-short.ts를 먼저 실행하세요.");
    process.exit(1);
  }

  const prompts: VideoPromptShort[] = JSON.parse(fs.readFileSync(PROMPTS_JSON, "utf-8"));

  if (!fs.existsSync(OUTPUT_DIR)) {
    fs.mkdirSync(OUTPUT_DIR, { recursive: true });
  }

  console.log(`\n🎬 Veo 3.1 숏폼 클립 비디오 생성 시작 (9:16)`);
  ...
  for (const vp of prompts) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (stat.size > 10000) existing.add(vp.clipId);
    }
  }

  if (existing.size > 0) {
    console.log(`⏭️  이미 생성된 클립 건너뛰기: ${[...existing].join(", ")}\n`);
  }
  ...
  for (let i = 0; i < prompts.length; i++) {
    ...
    if (existing.has(vp.clipId)) {
      console.log(`✅ [${i + 1}/${prompts.length}] ${vp.clipId} — 이미 존재, 건너뛰기`);
      successCount++;
      continue;
    }
    ...
    try {
      ...
      console.log(`  ✅ 저장 완료: ${vp.clipId}.mp4 (${(fileSize / 1024 / 1024).toFixed(1)}MB)`);
      successCount++;
    } catch (err: any) {
